...
function getTotalRevenue($conn) {
    $sql = "
        SELECT 
            COALESCE(SUM(CAST(p.cost AS DECIMAL(10,2))), 0.00) AS total_revenue 
        FROM 
            payment p
        JOIN 
...
function verifyRevenueConsistency($conn) {
    $sql = "
        SELECT 
            COUNT(*) as total_payments,
            SUM(p.cost) as total_payment_cost,
    ...
    error_log(sprintf(
        "Revenue Verification: Payments: %d, Payment Total: $%s, Service Total: $%s", 
        $row['total_payments'], 
        $row['total_payment_cost'], 
        $row['total_service_cost']
    ));
}

verifyRevenueConsistency($conn);
